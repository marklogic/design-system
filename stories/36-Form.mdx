import { Meta, Story, Preview, Props } from '@storybook/addon-docs/blocks';
import PropsPanels from './PropsPanels';
import MLForm from '../src/MLForm';

# MLForm

[Ant Documentation](https://3x.ant.design/components/form)

There's nothing here yet.


# MLForm.MLItem

There's nothing here yet.

## Usage Guidelines
### Horizontal Usage
* default use

### Inline Layout
* can be used if placed in header or space is a constraint

### Verticle Layout 
* can be used if horizontal space is limited

### Password Hints

![Password collapse](images/password_collapsed.png)

![Password expand](images/password_expanded.png)

### Inline Help
#### Form Field Tooltips
Use help icon, place it to the right of the form field:

![Form field tooltip](images/form_field_tooltip.png)

#### Info Popovers
Use help icon, place it to the right of the heading/text:

<img src="images/info_popover.png" width="500" />

### Input Fields
If an input field has no default value, it should include placeholder text, of the form "Enter ...".

![Input placeholder text](images/input_placeholder.png)

### Errors
Any errors should use the error color from our [color pallet](http://marklogic-ui-component-library.eng.marklogic.com/?path=/story/styles--color-palette).

#### Form Field Error
Highlight input field in error color, place error text below the field:

<img src="images/form_field_error.png" width="500" />

If the error is a missing required field, the text should be `<fieldname> is required`.

<img src="images/password_required.png" width = "300" />

#### Form field error with limited space (for example, inside a table cell)
Use error icon inside the field with error text on mouse-over:  

<img src="images/form_field_error_small.png" width="400" />

### Handling Required Fields 
If a form has one or more required fields, DO NOT disable the Save (or equivalent) button if they are empty. Instead, display an error message as described above when the user hits Save.

### Read-Only Forms
When entire form is read-only and the user doesn’t have permission to edit something, deal with displaying the edit form as follows:
* Do not show inputs
* Show field values in bold text
* Remove <img src="images/help.png" width="20" /> icons if they contain instructions how to fill in the form, keep <img src="images/help.png" width="20" /> icons if they provide helpful content about the fields.
* "Close Button" should be right-aligned with "info icon."
* The "info" icon, if provided for the field, should be 56 pixels to the right of the longest field value, and all other info icons should center-aligned with it.
* The text telling the user why they can't edit is at the bottom <position?>
* The exact text shown should follow the [Hints about Permission](http://marklogic-ui-component-library.eng.marklogic.com/?path=/docs/welcome--to-storybook) guideline.

<img src="images/Read-Only_Form.png" width="500" />

### Readonly Fields within Editable Forms
When an editable form has some fields that are not editable:
* The field should be disabled.
* Provide hover text  to explain why the field isn't editable.
  * If because of permissions, see the [Hints about Permission](http://marklogic-ui-component-library.eng.marklogic.com/?path=/docs/welcome--to-storybook#hints) for the appropriate text to use.
  * If for some other reason, state the reason.

<img src="images/Read-only_Field_With_Permission_Message.png" width="500" />
<img src="images/Read-only_Field_With_Specific_Permission_Message.png" width="500" />

## Text Guidelines
Use [Sentence case](https://elementary.io/docs/human-interface-guidelines#capitalization) for explanatory or instructional test.
For field labels:
* Do not capitalize all letters.
* Use [Title Case](https://elementary.io/docs/human-interface-guidelines#capitalization) for phrases. Do not use punctuation for phrases. 
Examples:

  **Bad**

  <img src="images/form_bad.png" width="500" />

  **Good**

  ![Form good](images/form_good.png)
  
* Use  [Sentence case](https://elementary.io/docs/human-interface-guidelines#capitalization) if the text is a complete sentence
* Avoid "number of." Use precise terms to make it clear that the table cell value will be a number. Examples:
  * **Bad**: Number of documents to process
  * **Good**: Batch Size
* Avoid question marks. Examples:
  * **Bad**: Thread Count?
  * **Good**: Thread Count
* Use short, precise terms whenever possible.
* Use [parallel structure](https://owl.purdue.edu/owl/general_writing/mechanics/parallel_structure.html) when listing radios or check box options. If all options have the same introductory text, move that text to the group label.

Examples:

**Bad**

  Search:
  - [ ] When searching an indexed database include company owner
  - [ ] When searching an indexed database include company name
  - [ ] When searching an indexed database include industry name
  
 **Good**
 
   When searching an indexed database, include:
  - [ ] Company Name
  - [ ] Company Owner
  - [ ] Industry Name



<PropsPanels of={[
  MLForm,
  MLForm.MLItem,
]} />
